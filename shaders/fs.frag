...
vec3 P(vec2 uv, float time, float fw) {
  ...
  vec3 shape =
      color * smoothstep(.1 + fw, .1 - fw, length(vec2(x * .7, y - .065)));
  shape -= Rect(vec2(x + .25, y + .2), vec2(.2, .4), fw);
  shape -= smoothstep(.065 + fw, .065 - fw, length(vec2(x * .8, y - .07)));
  shape = clamp(shape, .0, 1.);
...
vec3 Q(vec2 uv, float time, float fw) {
  ...
  vec3 shape = color * smoothstep(.13 + fw, .13 - fw, length(vec2(x, y * .8)));
  shape -= smoothstep(.09 + fw, .09 - fw, length(vec2(x2, y2 * .7)));
...
vec3 R(vec2 uv, float time, float fw) {
  ...
  vec3 shape =
      color * smoothstep(0.08 + fw, 0.08 - fw, length(vec2(x * .7, y - .065)));
  shape -= Rect(vec2(x + .22, y + .2), vec2(.2, .4), fw);
  shape -= smoothstep(0.065 + fw, 0.065 - fw, length(vec2(x * .8, y - .07)));
  shape = clamp(shape, .0, 1.);
...
vec3 Sl(vec2 uv, float time, float fw) {
  ...
  vec3 shape =
      color * smoothstep(fw, fw * 2., x * 1.7 - y * 1.3 + sin(y * 4.) * 1.3);
  shape -= smoothstep(.3 - fw, .3 + fw, x * 1.7 - y * 1.3 + sin(y * 4.) * 1.3);
...
vec3 T(vec2 uv, float time, float fw) {
  ...
  vec3 shape = color * Rect(vec2(x, y + .15), vec2(.04, .3), fw);
  shape += Rect(vec2(x + .09, y - .15), vec2(.21, .04), fw);
...
vec3 U(vec2 uv, float time, float fw) {
  ...
  vec3 shape = color * S(0.13 + fw, 0.13 - fw, length(vec2(x * .7, y * .4)));
  shape -= S(0.1 + fw, 0.1 - fw, length(vec2(x * .7, y * .4)));
...
void main() {
  ...
  shape *= P(vec2(uv.x * scale, uv.y * scale + sS * 16.), time, fw);
  shape *= Q(vec2(uv.x * scale, uv.y * scale + sS * 17.), time, fw);
  shape *= R(vec2(uv.x * scale, uv.y * scale + sS * 18.), time, fw);
  shape *= Sl(vec2(uv.x * scale, uv.y * scale + sS * 19.), time, fw);
  shape *= T(vec2(uv.x * scale, uv.y * scale + sS * 20.), time, fw);
  shape *= U(vec2(uv.x * scale, uv.y * scale + sS * 21.), time, fw);
  shape *= V(vec2(uv.x * scale, uv.y * scale + sS * 23.), time, fw);
